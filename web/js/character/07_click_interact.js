export default (function init(App) {
  ...
  App.animate = function animate() {
    // WebXR 会话中帧循环由 renderer.setAnimationLoop 驱动（XR 帧调度），跳过 rAF 自调度
    if (!App.xrPresenting) {
    ...
    // VR 注视引导（WebXR 沉浸模式）：视线与角色夹角<10° 稳定2秒 → 角色归位视线（可在相机设置中关闭）
    // 游戏模式 VR 由游戏接管移动/视角，跳过大厅的注视归位，避免干扰
    if (App.updateXrGazeAssist && !(App._xrGameMode && App.gameModeActive)) App.updateXrGazeAssist(dt);
    ...
    } else if (App.xrPresenting) {
      // WebXR 沉浸：相机由 WebXRManager 接管（头部追踪叠加在站位上），
      // 常规相机逻辑全部跳过；仅让角色平滑转向用户真实头部位置
      if (App.updateXRFaceUser) App.updateXRFaceUser();
    ...
    // 3D 背景模型：缓慢自转（VR 下保留，增强沉浸）+ 陀螺仪视差（VR 下跳过）
  ...
  App.animateModel = function animateModel(t, dt, mouthOpen) {
    ...
    if (!App.gameModeActive && App.idleWalkTarget && App.idleWalkProgress < 1) {
      ...
      // 线速度 = 段长 × idleWalkSpeed = 游戏移动速度 _moveSpeed（3.5），
      // 不再使用 smoothstep（其峰值速度达 1.5× 均值，超过游戏最大速度）；
      ...
      if (isInWalkAction && (Math.abs(dx) > 0.0001 || Math.abs(dz) > 0.0001)) {
        ...
        App.applyFullBodyWalkAnimation(dt, {
          ...
          stepLength: App._aiDrivenWalk ? (App.AI_LOBBY_WALK_STEP_LENGTH || 1.5) : undefined,
        });
      }
    } else {
      // 未在走路动作中，通知共享动画系统复位
    ...
    const baseTargetY = isTurning ? App.smoothRotY : (App.gameModeActive ? App.smoothRotY : camFaceY + App.smoothWalkFaceOff);
    ...
    } else if (!App.gameModeActive && walkProgress > 0 && App.idleWalkTarget) {
    ...
    if (App.vrShake && App.xrMode === 'webxr' && App.vrShake.leftRight > 0) {
      ...
      const swayAmp = 0.02 + vrS.leftRight * 0.0016; // 强度100 → ±0.18 摆动（幅度加倍）
    ...
    // 大厅 AI 行走线速度 = 段长 × idleWalkSpeed = 游戏速度 3.5 → 因子 1.167，与游戏完全相同
    ...
    if (!App.gameModeActive) {
      ...
      if (App._playerIsGrounded) {
        // AI 驱动行走步长与游戏内一致（1.5），弹跳步频/幅度与游戏移动相同
        ...
        if (App.vrShake && App.xrMode === 'webxr' && App.vrShake.upDown > 0) {
          const vrS = App.vrShake;
          const bounceAmp = 0.02 + vrS.upDown * 0.0018; // 强度100 → ±0.20 弹跳（幅度加倍）
