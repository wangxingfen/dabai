export default (function init(App) {
  ...
  App.showFloatingJoystick = function showFloatingJoystick(x, y) {
  ...
  App.updateFloatingJoystick = function updateFloatingJoystick(dx, dy) {
    const maxR = 50;
  ...
  App.onFPVKeyDown = function onFPVKeyDown(e) {
    if (!App.fpvMode) return;
    // 不拦截输入框中的按键（允许正常打字聊天）
  ...
  App.onFPVKeyUp = function onFPVKeyUp(e) {
    const k = e.key.toLowerCase();
    App.fpvKeys[k] = false;
  }; // 在 animate 中更新 FPV 相机
  App.updateFPVCamera = function updateFPVCamera(dt) {
    // 合并键盘 + 摇杆输入
    let forward = 0,
      strafe = 0;
