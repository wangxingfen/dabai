...
import { RLPersistence } from "./../game/rl/rl-persistence.ts";
 ...
 */
export function inputVecFromBridge(bridge: any, App: any): { x: number; z: number } | null {
...
export class HumanTrajectoryRecorder {
  /** 持久化层 */
  _persistence: RLPersistence;
  /** 当前采集会话 {gameKey, frames:[], startTs} */
  _recording: { gameKey: string; frames: Array<{ t: number; s: number[] | null; v: { x: number; z: number } }>; startTs: number } | null;
  /** 是否正在采集 */
  _active: boolean;
  /** 上帧时间戳 */
  _lastFrameTs: number;
  ...
  static _instance: HumanTrajectoryRecorder | null = null;

  /** 全局单例 */
  static get(): HumanTrajectoryRecorder {
  ...
  /** 开始采集 */
  startRecording(gameKey: string): boolean {
  ...
  /** 记录一帧（仅在用户操控期间调用） */
  recordFrame(game: any, bridge: any): void {
  ...
  /** 是否正在采集 */
  isRecording(): boolean { return this._active; }
   ...
   */
  async getTrajectories(gameKey: string): Promise<Array<{ gameKey: string; durationMs: number; frames: any[]; ts: number }>> {
    try {
      const keys = await this._persistence.listKeys('replays');
      const humanKeys = (keys || []).filter((k): k is string =>
  ...
  /** 统计某游戏的人类轨迹帧数 */
  async countFrames(gameKey: string): Promise<number> {
