export default (function init(App) {
  ...
  App.initTTSConfig = async function initTTSConfig() {
    ...
    App.ttsModal?.querySelectorAll('.tts-tab').forEach(tab => {
      tab.addEventListener('click', () => App.switchTTSEngine(tab.dataset.engine));
    });
    // 语速滑块实时显示
  ...
  App.applyTTSConfig = function applyTTSConfig(cfg) {
    App.currentTTSEngine = cfg.engine || 'edge_tts';
    App.ttsModal?.querySelectorAll('.tts-tab').forEach(t => {
      t.classList.toggle('active', t.dataset.engine === App.currentTTSEngine);
    });
    ...
    if (cfg.edge_rate) {
      ...
      if (App.ttsRateRange) App.ttsRateRange.value = pct;
  ...
  App.switchTTSEngine = function switchTTSEngine(engine) {
