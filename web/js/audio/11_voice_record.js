export default (function init(App) {
   ...
   */
  App._acquireMicStream = async function _acquireMicStream() {
    const isLive = (s) => s && s.active && s.getAudioTracks().some(t => t.readyState === 'live');
  ...
  App.startRecording = async function startRecording() {
    ...
    try {
      ...
      console.error('[Voice] 录音启动失败:', err.name, err.message);
