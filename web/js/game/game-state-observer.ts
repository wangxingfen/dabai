...
export class GameStateObserver {
  declare App: any;
  declare activeGame: any;              // 当前活跃的游戏实例
  declare snapshotInterval: number;     // 快照间隔(ms) - 每10秒发送完整状态（提升AI感知响应速度）
  declare _lastSnapshotTime: number;
  declare _processedEventCount: number;
  declare _sentGameContext: boolean;    // 是否已发送游戏上下文给AI

  // 非游戏模式快照
  declare _lobbyEnabled: boolean;       // 是否启用大厅模式快照
  declare _lobbyInterval: number;       // 大厅快照间隔 (ms)（低频，配合服务端主动说话闸门）
  declare _lastLobbySnapshotTime: number;

  // 用户（摄像机）位置追踪：用于计算摄像机的移动速度
  declare _lastUserPos: any;            // { x, z, t }
