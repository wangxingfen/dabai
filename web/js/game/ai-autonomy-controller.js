...
export class AIAutonomyController {
  ...
  receiveCommand(cmd) {
    ...
    // VR 模式（WebXR 沉浸）下拒绝所有 AI 自主移动指令：
    // 角色若按后端路径走动会径直走到相机位置，造成重叠/穿模
    if (this.App.xrMode && this.App.xrMode !== 'off') return;
  ...
  _injectWalkPath(path) {
    ...
    // 速度与游戏内移动一致：
    // - 游戏模式 → 游戏移动速度（_moveSpeed）
    // - 大厅 AI 驱动行走 → AI_LOBBY_WALK_SPEED（3.5，与游戏内移动相同）
    ...
    const gameSpeed = (this.App.gameModeActive && this.App.gameModeManager)
      ...
      : (this.App.AI_LOBBY_WALK_SPEED || 3.5);
   ...
   * @param {Object} gameInstance - 当前游戏实例（用于查地形）
   ...
   */
  update(dt) {
    // VR 模式下禁用 AI 自主移动（防止角色走动到相机位置）
    if (this.App.xrMode && this.App.xrMode !== 'off') {
      if (this._aiMoving) this._forceReset();
      return;
    }
