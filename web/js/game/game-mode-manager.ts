...
import { GameSceneGenerator } from './game-scene-generator.ts';
import { GameControlBridge } from './game-control-bridge.ts';
import { GameStateObserver } from './game-state-observer.ts';
...
export class GameModeManager {
  declare App: any;
  declare active: boolean;           // 是否在游戏模式中
  declare currentGame: any;          // 当前运行的游戏实例
  declare sceneGenerator: GameSceneGenerator;
  declare controlBridge: GameControlBridge;
  declare stateObserver: GameStateObserver;
  declare aiAutonomy: AIAutonomyController;

  // 订阅主程序动画循环
  declare _origAnimFrame: any;
  declare _gameLoopBound: any;
  declare _rafId: number | null;
  declare _lastGameLoopTime: number;

  // UI元素（延迟创建）
  declare _uiOverlay: HTMLElement | null;
  declare _uiPanel: any;
  declare _uiGameList: any;
  declare _uiScoreEl: HTMLElement | null;
  declare _uiTimerEl: HTMLElement | null;
  declare _uiHintEl: HTMLElement | null;
  declare _uiExitBtn: any;
  declare _settlementOverlay: HTMLElement | null;  // 结算界面
  declare _settlementShown: boolean;   // 是否已显示结算界面

  // AI 自动跳跃 + 瞬移兜底
  declare _aiLastStuckPos: any;     // { x, z } 上次记录位置（逐帧微停检测用）
  declare _aiJumpAttempts: number;  // 连续跳跃次数
  declare _aiStallOriginPos: any;   // { x, z } 开始卡住时的位置
  declare _aiStallStartTime: number; // 开始卡住的墙钟时间戳（毫秒）
  // AI 平滑攀爬过渡（替代瞬移）：以正常移动速度向目标位置过渡，避免视觉不适
  declare _aiClimbTarget: any;      // { x, z, startY, endY, totalDist } 攀爬目标
  declare _aiClimbCurrentPos: any;  // { x, y, z } 本帧攀爬位置（用于覆盖物理层对位置的修改）

  // 移动端控制
  declare _isMobile: boolean;
  declare _joystick: VirtualJoystick | null;           // 虚拟摇杆
  declare _touchState: any;         // 触控状态
  declare _mobileFullscreen: boolean; // 是否进入了全屏横屏

  // 相机与行走动画状态
  declare _savedCamera: any;
  declare _savedSceneObjects: any;
  declare _walkAnimTime: number;
  declare _walkAnimActive: boolean;
  declare _walkPhase: number;           // 行走相位 (rad)
  declare _prevAvatarPosY: number;      // 记录角色初始Y位置
  declare _WALK_STEP_LENGTH: number;    // 每步距离（控制步频），值越大步频越慢
  declare _mapRefreshTimer: number;
  declare _camAzimuth: number;           // 水平旋转角 (rad)，绕角色左右转
  declare _camPitch: number;             // 垂直俯仰角 (rad)，0=水平，正=上方俯视
  declare _camRadius: number;            // 距离（滚轮拉近推远），范围 1.5~15
  declare _camLookTarget: any;
  declare _camCurrent: any;
  declare _camSmooth: number;            // 平滑跟随速度
  declare _walkRampFactor: number;       // 移动渐强因子（AI/玩家共用）

  // 事件处理器（延迟创建/销毁）
  declare _onKeyDown: any;
  declare _onKeyUp: any;
  declare _gameDragInfo: any;
  declare _lastClickTime: number;        // 上一次点击时间，用于双击检测
  declare _clickTimeout: any;            // 单击延迟定时器（用于区分单击/双击）
  declare _onGamePointerDown: any;
  declare _onGamePointerMove: any;
  declare _onGamePointerUp: any;
  declare _onGameWheel: any;
  declare _hiddenElements: any;
  declare _onTouchStart: any;
  declare _onTouchMove: any;
  declare _onTouchEnd: any;
  declare _onTouchCancel: any;
  declare _settlementAutoExitTimer: any;
  ...
  _updateGameUI() {
    ...
    if (this._uiTimerEl) this._uiTimerEl.textContent = String(Math.floor(this.currentGame.elapsedTime));
    if (this._uiHintEl) {
  ...
  _showGameSelection() {
    ...
    overlay.addEventListener('click', (e) => {
      if (e.target === overlay) closeSelection();
    });
    overlay.querySelector('.game-select-cancel').addEventListener('click', closeSelection);
    document.addEventListener('keydown', onKeydown);

    overlay.querySelectorAll('.game-select-card').forEach(card => {
      card.addEventListener('click', () => {
        closeSelection();
        self.enterGameMode((card as HTMLElement).dataset.key);
      });
    });
  }
}
