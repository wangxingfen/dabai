...
import { oneHot } from './world-model.ts';
 ...
 */
export function segmentFeature(seg: Segment, stateSize: number, nActions: number): Float64Array {
  const sArr = seg.s || [];
...
/** 片段形状：s/a/r 均可为数组（多帧）或标量（单帧） */
type Segment = { s?: Float64Array | number[]; a?: number | number[] | null; r?: number | number[] | null };
export class RLHFLite {
  ...
  w: Float64Array;
  /** 偏好对池 */
  pairs: { g: Float64Array; b: Float64Array }[];
  /** 统计 */
  stats: { pairs: number; updates: number; trainLoss: number; sep: number };
   ...
   */
  constructor(opts: { stateSize?: number; nActions?: number } = {}) {
   ...
   */
  addPair(good: Segment, bad: Segment): boolean {
   ...
   */
  addPairs(list: Array<{ good: Segment; bad: Segment }> = []): number {
    let n = 0;
    for (const p of list) {
  ...
  addFeaturePair(gFeat: ArrayLike<number>, bFeat: ArrayLike<number>): void {
    this.pairs.push({ g: Float64Array.from(gFeat), b: Float64Array.from(bFeat) });
    if (this.pairs.length > MAX_PAIRS) this.pairs.shift();
    this.stats.pairs = this.pairs.length;
  }

  _feat(seg: Segment | null): Float64Array | null {
   ...
   */
  train(epochs: number = PREF_EPOCHS): { loss: number; sep: number } {
  ...
  _logit(feat: ArrayLike<number>): number {
    let s = 0;
   ...
   */
  shaping(stateVec: Float64Array | number[], actionId: number, rewardHint: number = 0): number {
   ...
   */
  actionBias(stateVec: Float64Array | number[], rewardHint: number = 0): Float64Array | null {
