 ...
 * @returns {string}
 */
export function bucketKey(stateVec, bits = DEFAULT_BUCKET_BITS) {
  const levels = 2 ** bits;
  let k = '';
 ...
 */
export function inputVecToAction(inputVec, actionSpec) {
...
export function softmaxPolicy(qValues, temperature = 1.0) {
...
export class BehaviorCloningPrior {
  ...
  /** 已吸收的有效 (s,a) 对数量 */
  get pairCount() { return this._pairs.length; }

  /** KL 历史（评估/仪表用） */
  get klHistory() { return this._klHistory.slice(); }

  get lastKL() { return this._lastKL; }
   ...
   * @returns {number}
   */
  measureKL(agentPolicyFn, sampleStates) {
