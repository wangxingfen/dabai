...
/** one-hot 编码动作 */
export function oneHot(actionId, nActions) {
...
export class WorldModel {
  ...
  train(batchSize = 32) {
    ...
    const inputs = [], targets = [], stateTargets = [], rewardTargets = [];
    for (let i = 0; i < bs; i++) {
   ...
   */
  imagineRollout(seedStates = [], policyFn = null, horizon = 8) {
   ...
   * @returns {Object}
   */
  sampleEfficiencyReport(steps = 10) {
    const n = this._pairs.length;
