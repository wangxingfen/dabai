...
import { encodeObservation, observationDim } from './observation-spec.js';
import { generateLevel, generateSeeds, describeLevel } from './level-procedural.js';
...
export class ZeroShotEvaluator {
  ...
  async evaluate(weights, cfg, policyFn = null) {
    ...
    const net = (weights && !policyFn)
      ? (() => { const n = new NeuralNetV2([cfg.stateSize, ...(cfg.hiddenLayers || [16, 16]), cfg.nActions], { lr: 0, noisy: false, seed: 1 }); n.fromJSON(weights); return n; })()
      : null;
    ...
    for (const seed of seeds) {
      ...
      const levelResult = { seed, note: describeLevel(level), wins: 0, episodes: 0, steps: [], progresses: [], hitWallRates: [] };
