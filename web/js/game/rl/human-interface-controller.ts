...
export class HumanInterfaceController {
  ...
  _lastAct: number;
  /** 当前待生效的延迟（ms） */
  _nextDelay: number | null;
  /** 最近延迟记录（频率统计窗口） */
  _recentDelays: number[];
  /** 累计决策次数 */
  _decisions: number;
  ...
  constructor({ minDelayMs = DEFAULT_MIN_DELAY_MS,
                ...
                maxRateHz = DEFAULT_MAX_RATE_HZ }: { minDelayMs?: number; maxDelayMs?: number; maxRateHz?: number } = {}) {
   ...
   */
  setLatencyProfile({ minMs, maxMs, maxRateHz }: { minMs?: number; maxMs?: number; maxRateHz?: number } = {}): void {
  ...
  /** 累计决策次数 */
  get decisionCount(): number { return this._decisions; }
}
