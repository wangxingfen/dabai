...
import { getGameConfig } from '../games/games-config.js';
import { HumanInterfaceController } from './human-interface-controller.js';
import { HumanBaselineEvaluator } from './human-baseline-eval.js';
import { HumanTrajectoryRecorder } from '../../input/human-trajectory-recorder.js';
import { BehaviorCloningPrior } from './behavior-cloning.js';
import { WorldModel } from './world-model.js';
import { RLHFLite } from './rlhf-lite.js';
...
export class RLAgentManager {
   ...
   */
  async warmStart(fromKey, toKey, game = null) {
  ...
  getZeroShotEvaluator() {
    if (!this._zeroShotEval) {
      const mod = import('./zero-shot-eval.js');
      // 惰性动态加载，避免启动时引入评测依赖
   ...
   */
  async runZeroShotEval(gameKey, opts = {}) {
    const agent = this._agents.get(gameKey);
    if (!agent) return null;
    const mod = await import('./zero-shot-eval.js');
