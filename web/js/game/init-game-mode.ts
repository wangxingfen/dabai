...
import { GameModeManager } from "./game-mode-manager.ts";
import { TreasureHuntGame } from './games/treasure-hunt.ts';
import { SandboxGame } from './games/sandbox-game.ts';
import { MobaGame } from './games/moba-5v5.ts';
import { MarioGame } from './games/mario-game.ts';
import { XiangqiGame } from "./games/xiangqi-game.ts";
// 缓存失效由 Vite 接管（dev 走协商缓存，build 产物带内容哈希），不再使用 ?v= 查询参数
import { CyberCorpGame } from "./games/cyber-corp.ts";
import { GAME_CONFIGS } from './games/games-config.ts';
...
export default function init(App: AppKernel) {
  ...
  if (lockModeBtn) {
    toolsBar.insertBefore(gameBtn, lockModeBtn);
  } else {
