...
export class XiangqiGame extends BaseGame {
  ...
  getObservationSpec() {
    const spec: { name: string; kind: string; shape?: number[]; scale: number; offset: number }[] = [{ name: 'board', kind: 'grid', shape: [BOARD_COLS * BOARD_ROWS], scale: 14, offset: 0 }];
    const extras = [
  ...
  _updatePanelUI() {
    ...
    if (!p) return;
    const setEl = (id, val?) => {
      const el = p.querySelector(`#${id}`) as HTMLElement | null;
    ...
    const autoStatusEl = p.querySelector('#xiangqi-rl-auto') as HTMLElement | null;
    if (autoStatusEl) autoStatusEl.style.color = this._rl.autoPlay ? '#00ff88' : '#666';
    setEl('xiangqi-rl-llm', this._llm.enabled ? '✅ 已开启' : '关闭');
    const llmStatusEl = p.querySelector('#xiangqi-rl-llm') as HTMLElement | null;
    if (llmStatusEl) llmStatusEl.style.color = this._llm.enabled ? '#00ff88' : '#666';

    // 更新按钮文字
    const rlBtn = p.querySelector('#xiangqi-btn-rl') as HTMLElement;
    ...
    const autoBtn = p.querySelector('#xiangqi-btn-auto') as HTMLElement;
    ...
    const llmBtn = p.querySelector('#xiangqi-btn-llm') as HTMLElement;
  ...
  declare THREE: any;
  declare _anim: any;
  declare _boardGroup: any;
  declare _boardPlane: any;
  declare _boundPointerDown: any;
  declare _boundPointerUp: any;
  declare _canvas: any;
  declare _disposeObject: any;
  declare _dragCleanup: any;
  declare _engine: any;
  declare _getPlayerFacing: any;
  declare _getPlayerPosition: any;
  declare _getRecentEvents: any;
  declare _isAnimating: any;
  declare _lastMove: any;
  declare _lastMoveHighlights: any;
  declare _llm: any;
  declare _moveCount: any;
  declare _moveIndicators: any;
  declare _observerSide: any;
  declare _panel: any;
  declare _pieceMeshes: any;
  declare _pieceTextures: any;
  declare _playerSpeed: any;
  declare _pointerDownInfo: any;
  declare _raycaster: any;
  declare _rl: any;
  declare _selectedPiece: any;
  declare _selectedRing: any;
  declare _statusBar: any;
  declare _turnTimer: any;
  declare _validMoves: any;
  declare _waitingForRobot: any;
