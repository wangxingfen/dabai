...
class CharacterActor {
  ...
  declare App: any;
  declare THREE: any;
  declare _anchorLogN: any;
  declare _anchorTotal: any;
  declare _blinkPhase: any;
  declare _blinkT: any;
  declare _blinking: any;
  declare _bubbleCanvas: any;
  declare _bubbleCtx: any;
  declare _bubbleH: any;
  declare _bubbleOpacity: any;
  declare _bubblePop: any;
  declare _bubbleShowT: any;
  declare _bubbleTex: any;
  declare _bubbleVisible: any;
  declare _bubbleW: any;
  declare _emotionKey: any;
  declare _exprCur: any;
  declare _falling: any;
  declare _fitErr: any;
  declare _footToGroup: any;
  declare _holoActive: any;
  declare _lastStepKey: any;
  declare _motionActive: any;
  declare _motionCur: any;
  declare _motionDef: any;
  declare _motionElapsed: any;
  declare _motionIdleInterval: any;
  declare _motionIdleT: any;
  declare _motionName: any;
  declare _onArrive: any;
  declare _phase: any;
  declare _tmpFootV3: any;
  declare _walkPhase: any;
  declare bubble: any;
  declare card: any;
  declare exprNames: any;
  declare faceTarget: any;
  declare footBone: any;
  declare game: any;
  declare group: any;
  declare hiredPos: any;
  declare isMoving: any;
  declare isSpeaking: any;
  declare loaded: any;
  declare name: any;
  declare rate: any;
  declare removed: any;
...
export class CyberCorpGame extends BaseGame {
  ...
  _makeTextPlane(text, opts: { width?: number; height?: number; font?: string; glow?: string; color?: string; width3D?: number; height3D?: number } = {}) {
    const THREE = this.THREE;
  ...
  _buildWorkstations() {
    ...
    for (const ws of LAYOUT.workstations) {
      ...
      // 记录房间牌引用（入职后更新为员工名）
      (ws as any).label = label;
      (ws as any).posDef = posDef;
    }
  }
  ...
  _startCEOOnboarding() {
    ...
    this._ceoUI = root;
    const commit = () => {
      const name = ((root.querySelector('#cc-ceo-name') as HTMLInputElement).value || defName).trim().slice(0, 12) || defName;
      const company = ((root.querySelector('#cc-ceo-company') as HTMLInputElement).value || '赛博公司').trim().slice(0, 16) || '赛博公司';
      const motto = ((root.querySelector('#cc-ceo-motto') as HTMLInputElement).value || '').trim().slice(0, 40);
      if (this._disposed) return;
    ...
    root.querySelector('#cc-ceo-rnd').addEventListener('click', () => {
      (root.querySelector('#cc-ceo-name') as HTMLInputElement).value = pool[Math.floor(Math.random() * pool.length)];
    });
    (root.querySelector('#cc-ceo-name') as HTMLInputElement).focus();
  }
    ...
    try {
      ...
      const save = {
        ...
        rl: Object.fromEntries(
          Object.entries(this._rl || {}).map(([n, a]) => [n, (a as any).snapshot()])
        ),
  ...
  _showHRSelectUI(cards) {
    ...
    const setMode = (m) => {
      mode = m;
      root.querySelectorAll('.cyber-hr-mode').forEach(b => b.classList.toggle('active', (b as HTMLElement).dataset.mode === m));
    };
    root.querySelectorAll('.cyber-hr-mode').forEach(b => b.addEventListener('click', () => setMode((b as HTMLElement).dataset.mode)));
    ...
    root.querySelectorAll('.cyber-hr-card').forEach(el => {
      el.addEventListener('click', () => pick(cards[+(el as HTMLElement).dataset.i]));
    });
  ...
  async _hire(pos, cand, result) {
    ...
    // 更新岗位牌
    if ((ws as any).label) {
      this._setLabelText((ws as any).label, `${pos.icon} ${cand.name}`, pos.color);
    }
    this._updatePositionsUI();
  ...
  async _assignPlayerTask(text) {
    ...
    this._playerTask.subtasks = posKeys.map(posKey => {
      ...
      const template = PROJECT.SUBTASK_TEMPLATE[posKey as keyof typeof PROJECT.SUBTASK_TEMPLATE] || '子任务';
      return {
        posKey,
        icon: pos.icon,
  ...
  _layoffEmployee(actor) {
    ...
    // 更新岗位牌（恢复岗位名）
    if (ws && (ws as any).label) {
      this._setLabelText((ws as any).label, `${pos.icon} ${pos.name}`, pos.color);
    }
    this._updatePositionsUI();
  ...
  _showResult(score, pass, pos, cand) {
    ...
    requestAnimationFrame(() => requestAnimationFrame(() => {
      ...
      if (bar) (bar as HTMLElement).style.width = score + '%';
    }));
    const t = setTimeout(() => {
  ...
  async _drawScoreDashboard(ms) {
    ...
    for (const [a, b, color, name] of zones) {
      const x0 = cx0 + ((a as number) / 6) * curveW, x1 = cx0 + ((b as number) / 6) * curveW;
      ctx.fillStyle = color;
  ...
  declare App: any;
  declare THREE: any;
  declare _addedLights: any;
  declare _agentChat: any;
  declare _agentEnergy: any;
  declare _agentProgress: any;
  declare _agentTask: any;
  declare _airJumps: any;
  declare _audios: any;
  declare _blockers: any;
  declare _candVisible: any;
  declare _ceoUI: any;
  declare _chatCooldown: any;
  declare _chatDebounce: any;
  declare _chatHookActive: any;
  declare _chatPanelEl: any;
  declare _currentPos: any;
  declare _directHireMode: any;
  declare _directHiring: any;
  declare _directMask: any;
  declare _directPosKey: any;
  declare _discussionActive: any;
  declare _disposed: any;
  declare _fillerIdx: any;
  declare _firePending: any;
  declare _forceRefine: any;
  declare _gameLoopRunning: any;
  declare _ghostAnchor: any;
  declare _holoCanvas: any;
  declare _holoCtx: any;
  declare _holoMesh: any;
  declare _holoPlaceholder: any;
  declare _holoTexture: any;
  declare _hookFn: any;
  declare _hrSelectUI: any;
  declare _hudT: any;
  declare _isGrounded: any;
  declare _lastReviewAt: any;
  declare _lastRewardSign: any;
  declare _lastSettle: any;
  declare _lastSettleEvents: any;
  declare _llmFailCount: any;
  declare _llmOk: any;
  declare _lobbyAvatarRemoved: any;
  declare _meetingActive: any;
  declare _meetingSpots: any;
  declare _nearAgentName: any;
  declare _neonLights: any;
  declare _nextErrandAt: any;
  declare _nextReportAt: any;
  declare _nextSyncAt: any;
  declare _opPanel: any;
  declare _origAddUserMsg: any;
  declare _origSendText: any;
  declare _particlePos: any;
  declare _particles: any;
  declare _playerControlledHR: any;
  declare _playerTask: any;
  declare _playerVelocityY: any;
  declare _reportActive: any;
  declare _resumeInterviewNotice: any;
  declare _reviewPending: any;
  declare _rl: any;
  declare _rlLearnCount: any;
  declare _rlNextAct: any;
  declare _running: any;
  declare _savedAvatarVisible: any;
  declare _savedFog: any;
  declare _sendHook: any;
  declare _skipWelcome: any;
  declare _speakSeq: any;
  declare _statusTimer: any;
  declare _stoppedHiring: any;
  declare _stuckStreak: any;
  declare _styleEl: any;
  declare _swarmBusy: any;
  declare _swarmCollabCount: any;
  declare _swarmCooldown: any;
  declare _swarmCycle: any;
  declare _swarmDecisions: any;
  declare _swarmRunning: any;
  declare _swarmTickInterval: any;
  declare _swarmTickT: any;
  declare _tasks: any;
  declare _timers: any;
  declare _transcript: any;
  declare _uiCandName: any;
  declare _uiCandPos: any;
  declare _uiCandWrap: any;
  declare _uiCandidate: any;
  declare _uiPosChips: any;
  declare _uiPositions: any;
  declare _uiResult: any;
  declare _uiRoot: any;
  declare _uiStatus: any;
  declare _uiSwarmList: any;
  declare _uiTrans: any;
  declare _vrBtnEl: any;
  declare _worldTick: any;
  declare _xrActive: any;
  declare _xrTurnYaw: any;
  declare _xrWrap: any;
  declare actors: any;
  declare boundarySize: any;
  declare cards: any;
  declare ceo: any;
  declare currentSeeker: any;
  declare description: any;
  declare displayName: any;
  declare elapsedTime: any;
  declare filledPositions: any;
  declare fpvCamera: any;
  declare fpvEyeHeight: any;
  declare hiredList: any;
  declare hrActor: any;
  declare hrCard: any;
  declare initialCameraHeight: any;
  declare initialCameraRadius: any;
  declare isIsolated: any;
  declare moveSpeed: any;
  declare name: any;
  declare phase: any;
  declare playerAnchor: any;
  declare sceneObjects: any;
  declare score: any;
  declare seekerQueue: any;
  declare state: any;
  declare stats: any;
  declare uiHint: any;
  declare worldId: any;
