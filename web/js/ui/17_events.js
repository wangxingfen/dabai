export default (function init(App) {
  ...
  App.updateCameraSettingsUI = function updateCameraSettingsUI() {
    ...
    if (App.camDistanceRange) {
      App.camDistanceRange.value = App.cameraDistance;
  ...
  App.bindEvents = function bindEvents() {
    ...
    // VR 注视归位开关：初始化勾选状态 + 变更即保存
    if (App.vrGazeAssistToggle) {
      App.vrGazeAssistToggle.checked = !!App.gazeAssistEnabled;
      App.vrGazeAssistToggle.addEventListener('change', () => {
        App.gazeAssistEnabled = App.vrGazeAssistToggle.checked;
        App.saveCameraSettings();
        App.showToast(App.gazeAssistEnabled ? 'VR 注视归位已开启' : 'VR 注视归位已关闭');
      });
    }
    if (App.camHeightRange) {
    ...
    App.updateVRShakeNotify = function updateVRShakeNotify() {
      ...
      let core = App._datingSystem || App._engagementRL;
