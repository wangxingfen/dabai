...
9. **快而好的节奏**：先打通主干再打磨细节，收益递减就停；执行期间少说多干——
   不播报进展、不口头同步，只在出结论、卡住或需要用户决策时说话。
...
> **2026-08-30 收敛机制**：`on_demand` 技能的工具注册是**按轮有效**的——
> 每轮对话开始时只保留基础工具（`skill_help` + full 技能），上一轮激活的
> 技能工具全部卸载（`agent._prune_tools_for_new_turn`）；单轮内注册总量
> 有上限（`settings.json -> agent.max_active_tools`，默认 48），超出按
> 「最久未使用」逐技能淘汰。这样多轮对话不会把工具定义累积成"全额加载"，
> 渐进披露真正做到上下文有界。
